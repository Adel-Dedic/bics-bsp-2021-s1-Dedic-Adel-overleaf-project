...
Some statistics and comparison of probabilities and mathematics is generally used in the
scientific report which requires some mathematical background to fully comprehend.
...
\label{sec-production}
\subsection{Requirements ($\pm$ 15\% of section's words)}
The program shall behave like the original Enigma Machine did, this includes being able to encrypt and to decrypt messages with the same 'key' provided previously.\\
Another key features from the original that is included are fully functional rotors with which the user is able to configure manually (not random)\\
Furthermore, one main difference compared to its predecessors is the inclusion of encryption/decryption of a whole string of text instead of a key-by-key method\\
Finally, an option to export and import a text file which possesses the key information to ease the usability of the program.
\subsection{Design ($\pm$ 30\% of section's words)}
...
Finally the given buttons at the end are self-explanatory by their name. i.e.: 'Encrypt', 'Decrypt', 'Export*' and 'Import*'\\
...
Since the intention of the program was to implement the functions of the original Enigma Machine thus it required to develop a way to transform 'mechanics' to 'code' basically.\\
This was done through dissecting the Enigma into different parts for the program.\\
