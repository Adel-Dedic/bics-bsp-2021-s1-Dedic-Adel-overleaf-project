...
\subsection{Design}
The scientific deliverable is going to start off with a brief description of what Enigma is and introduce encryption which is also further explained at a later point. With that out of the way, the main part of the report is coming up explaining the flaws of Enigma.\\
...
Finally where none of the letters match up, which means that the encoding works in this case. Take in consideration that the word 'Windy' isn't guaranteed that it is in the secret message itself but it gives us a bigger picture understanding where to start looking for breaking messages. Figure \ref{fig:test}\\
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale = .7]{interface.png}
    \caption{Interface}
    \label{fig:infac}
\end{figure}
% that's all folks
